...
const { MODELS, REVIEW_SETTINGS, hash, validateReview, validateModelSet, loadExperimentCore } = require('./eval_consolidation_matched');
...
function buildReport(root) {
    ...
    if (JSON.stringify(fixture.reviewSettings) !== JSON.stringify(REVIEW_SETTINGS)) throw new Error('Frozen reviewer settings mismatch');
    ...
    for (const item of fixture.items) {
        ...
        if (JSON.stringify(reviewed.settings) !== JSON.stringify(REVIEW_SETTINGS)) throw new Error('Reviewer settings mismatch');
    ...
    return { experiment: fixture.experiment, completedAt: completion.completedAt, spent: completion.spent,
        ...
        limitations: ['Five threads, one generation per model per thread; repeatability and article quality are not measured.',
...
function markdown(report) {
    ...
    lines.push('', ...Object.entries(report.definitions).map(([key, value]) => `- **${key}:** ${value}`), '', ...report.limitations.map(text => '- ' + text), '',
        `Completed ${report.completedAt}. Total new evaluation spend: $${report.spent.toFixed(6)}. Raw requests, responses and frozen sources: outputs/consolidation-matched/.`, '',
        ...
        ...(report.adjustments || []).map(change => `Protocol adjustment before any completed grade: ${change.reason} Cost of the ungraded attempt is included in total evaluation spending.`), '',
