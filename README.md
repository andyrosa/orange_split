...
| Haiku 4.5 / GLM 5.3 Flash | 484 | 64 | 42 | 22 | 19 | 238 (0.49) | 0.50 | $0.43 | 1017 s |
...
- Consolidators on the same 64 Haiku candidates: Haiku 4.5 barely merges (63 axes, 33 one-arm); GLM 5.3 Flash gave 22 two-sided of 42 with 19 one-arm for $0.02, but its consolidation call took 17 minutes, and at 50% it gave fewer two-sided rows than Sonnet; Sol low cost $0.03 and 38 s but 16 two-sided of 42 with 23 one-arm; Luna max matched Sonnet (20 two-sided of 36) at $0.08 but took 703 s and 67k completion tokens; Fable 5.1 low took 56 s and 4.9k tokens for $0.31, below Sonnet on every quality number.
...
Per-stage figures per 1000 comment tokens: Haiku extraction and scoring $0.011 (matching the $0.0119 measured on the test thread), Luna $0.0033 at 1.2 s, Opus $0.132 at 3.2 s (Haiku 0.5 s here, 0.87 s on the test thread, which its label uses); consolidation Sol low $0.0010 at 1.2 s, Luna max $0.0025 at 21.6 s, Fable low $0.0096 at 1.7 s. Sonnet 5 consolidation took 497 s and 54.7k completion tokens on Opus's 134 candidates against 195 s and 22.8k on Haiku's 64, so the forecast scales the consolidation share by a per-model candidate factor (Opus 5 2.1, GLM 5.3 Flash 1.3, others 1). The page's forecast for the test thread snapshot with Opus 5 and Sonnet 5 is $13.85 and 14 minutes per run.

## Pareto frontier

Objectives: quality (two-sided axes, tie-break agreed stances per comment), cost per run, wall time. One representative run per configuration; both GLM values where two runs exist. The two rows marked as page forecasts were not run on the test thread: their cost and time are what the page forecasts for its snapshot, so they match what the page shows.
...
| Opus 5 + Sonnet 5 (page forecast) | about 3 times Haiku + Sonnet 5's | about 0.73 | $13.85 | 14 min | yes: best quality by a wide margin, at the highest cost |
| GPT-5.6 Luna low + Sonnet 5 (page forecast) | about 1.5 times Haiku + Sonnet 5's | about 0.51 | $0.62 | 6 min | yes: better and cheaper than Haiku + Sonnet 5, slower |
