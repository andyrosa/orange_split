...
Per axis: statement 1 is blue at 18px and statement 2 is orange at 14px. These sizes are fixed for every axis. Between them is a fixed-width proportion bar, blue for statement 1, grey for middle, and orange for statement 2, showing the commenter split within the axis rather than its total size. The side counts flank the bar, followed on the same line by the agreed comment classifications, total commenters, and middle count when above zero. The runner prints the counts in words.
