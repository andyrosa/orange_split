...
A table with one row per axis, many rows (target 30 or more), sorted so the most polarizing axes are at the top. "Polarizing" means commenters land on both statements with little middle. Each row also carries a consensus score, so that near-unanimous axes with enough signal can be surfaced by re-sorting, plus coverage, the split between the two statements, an error bar from the two scoring passes, and the nearest related axis. Axes on which nearly everyone agrees sort low under polarization even if many comments touch them.
...
2. Move the "Comments to analyze" slider. It keeps the first N% of comments in thread order, which is HN's ranking, and since ancestors precede replies every kept reply still has its parent for context. The text beside it shows how many comments that leaves, and the estimate line shows the approximate cost and time for the selected models, computed from per-token rates and stage times fitted to the measured runs.
...
Every button sits right after the thing it acts on and appears only when it can do something: "Load thread" after the Thread box while the box does not denote the loaded thread, "Refresh thread" after the snapshot timestamp while that thread has a snapshot, "Run" after the key field while idle and "Cancel" in its place while a run is in progress, "Delete cached data" after the storage line while cached data exists, and "Wipe OpenRouter key" beside it while a key is stored.
...
- Nearest axis: the axis whose commenters overlap most in a consistent direction, by absolute cosine similarity of per-commenter stance vectors (+1 statement 1, -1 statement 2), with the cosine and the number of shared commenters.

Clicking a header sorts by that column; the Split header sorts by balance.
